...
const ChatView: React.FC<ChatViewProps> = ({ studentName, initialWorry, counselorType, onFinish, isFinishing }) => {
  ...
  const startChat = useCallback(async () => {
    ...
    const userMessage: ChatMessage = { role: 'user', content: initialWorry };
    const modelPlaceholder: ChatMessage = { role: 'model', content: '' };
    setMessages([userMessage, modelPlaceholder]);
    ...
    try {
      ...
      const errorMessage = '죄송해요, 지금은 연결이 불안정해요. 잠시 후 다시 시도해주세요.';
      setMessages(prev => {
        const updatedMessages = [...prev];
        if(updatedMessages.length > 0 && updatedMessages[updatedMessages.length - 1].role === 'model') {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        const responseStream = await chatSession.sendMessageStream({ message: messageToSend });
        let streamedResponse = "";
        for await (const chunk of responseStream) {
            streamedResponse += chunk.text;
            setMessages(prev => {
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-md lg:max-w-lg px-4 py-3 rounded-2xl ${msg.role === 'user' ? 'bg-blue-500 text-white rounded-br-none' : 'bg-slate-100 text-slate-800 rounded-bl-none'}`}>
              <p className="whitespace-pre-wrap">{msg.content || (isLoading && <LoadingSpinner className="w-5 h-5 text-slate-500" />)}</p>
            </div>
